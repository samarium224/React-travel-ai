...
export default function Home() {
  ...
  // Render itinerary including nearest hotels
const renderItinerary = () => {
  if (!itinerary) return null;

  const days = Object.keys(itinerary).filter(key => key !== "Nearest Hotels");

  return (
    <div>
      {days.map(day => (
        <div key={day} className="mb-6">
          <h2 className="text-3xl font-bold text-gray-800 mb-4">{`Day ${day}`}</h2>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {itinerary[day].map((item, index) => (
              <div key={index} className="bg-white shadow-md rounded-lg p-4">
                <h3 className="text-xl font-semibold text-gray-700">{item.place}</h3>
      ...
      {/* Nearest Hotels */}
      {itinerary["Nearest Hotels"] && (
        <div className="mb-6">
          <h2 className="text-3xl font-bold text-gray-800 mb-4">Nearest Hotels</h2>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {itinerary["Nearest Hotels"].map((hotel, index) => (
              <div key={index} className="bg-white shadow-md rounded-lg p-4">
                <h3 className="text-xl font-semibold text-gray-700">{hotel[0]}</h3>
                <p className="text-gray-600">Distance: {hotel[1].toFixed(2)} km</p>
